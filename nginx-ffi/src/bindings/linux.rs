...
pub const NGX_COMPILER: &'static [u8; 28usize] = b"gcc 8.3.0 (Debian 8.3.0-6) \0";
...
impl ::std::fmt::Debug for ngx_log_s {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! ( f , "ngx_log_s {{ file: {:?}, connection: {:?}, handler: {:?}, data: {:?}, writer: {:?}, wdata: {:?}, action: {:?}, next: {:?} }}" , self . file , self . connection , self . handler , self . data , self . writer , self . wdata , self . action , self . next )
    }
}
extern "C" {
...
pub struct ngx_sockaddr_t {
    ...
    pub bindgen_union_field: [u32; 28usize],
}
...
impl ::std::fmt::Debug for ngx_connection_s {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! ( f , "ngx_connection_s {{ data: {:?}, read: {:?}, write: {:?}, fd: {:?}, recv: {:?}, send: {:?}, recv_chain: {:?}, send_chain: {:?}, listening: {:?}, log: {:?}, pool: {:?}, type: {:?}, sockaddr: {:?}, addr_text: {:?}, proxy_protocol: {:?}, udp: {:?}, local_sockaddr: {:?}, buffer: {:?}, queue: {:?}, number: {:?}, buffered : {:?}, log_error : {:?}, timedout : {:?}, error : {:?}, destroyed : {:?}, idle : {:?}, reusable : {:?}, close : {:?}, shared : {:?}, sendfile : {:?}, sndlowat : {:?}, tcp_nodelay : {:?}, tcp_nopush : {:?}, need_last_buf : {:?} }}" , self . data , self . read , self . write , self . fd , self . recv , self . send , self . recv_chain , self . send_chain , self . listening , self . log , self . pool , self . type_ , self . sockaddr , self . addr_text , self . proxy_protocol , self . udp , self . local_sockaddr , self . buffer , self . queue , self . number , self . buffered ( ) , self . log_error ( ) , self . timedout ( ) , self . error ( ) , self . destroyed ( ) , self . idle ( ) , self . reusable ( ) , self . close ( ) , self . shared ( ) , self . sendfile ( ) , self . sndlowat ( ) , self . tcp_nodelay ( ) , self . tcp_nopush ( ) , self . need_last_buf ( ) )
    }
}
...
impl ::std::fmt::Debug for ngx_event_s {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! ( f , "ngx_event_s {{ data: {:?}, write : {:?}, accept : {:?}, instance : {:?}, active : {:?}, disabled : {:?}, ready : {:?}, oneshot : {:?}, complete : {:?}, eof : {:?}, error : {:?}, timedout : {:?}, timer_set : {:?}, delayed : {:?}, deferred_accept : {:?}, pending_eof : {:?}, posted : {:?}, closed : {:?}, channel : {:?}, resolver : {:?}, cancelable : {:?}, available: {:?}, handler: {:?}, log: {:?}, timer: {:?}, queue: {:?} }}" , self . data , self . write ( ) , self . accept ( ) , self . instance ( ) , self . active ( ) , self . disabled ( ) , self . ready ( ) , self . oneshot ( ) , self . complete ( ) , self . eof ( ) , self . error ( ) , self . timedout ( ) , self . timer_set ( ) , self . delayed ( ) , self . deferred_accept ( ) , self . pending_eof ( ) , self . posted ( ) , self . closed ( ) , self . channel ( ) , self . resolver ( ) , self . cancelable ( ) , self . available , self . handler , self . log , self . timer , self . queue )
    }
}
impl ngx_event_s {
    #[inline]
...
impl ::std::fmt::Debug for ngx_http_upstream_local_t {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write!(
            f,
            "ngx_http_upstream_local_t {{ addr: {:?}, value: {:?} }}",
...
impl ::std::fmt::Debug for ngx_http_upstream_rr_peer_s {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! ( f , "ngx_http_upstream_rr_peer_s {{ sockaddr: {:?}, name: {:?}, server: {:?}, lock: {:?}, next: {:?} }}" , self . sockaddr , self . name , self . server , self . lock , self . next )
    }
}
...
impl ::std::fmt::Debug for ngx_http_upstream_rr_peers_s {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! ( f , "ngx_http_upstream_rr_peers_s {{ shpool: {:?}, rwlock: {:?}, zone_next: {:?}, single : {:?}, weighted : {:?}, name: {:?}, next: {:?}, peer: {:?} }}" , self . shpool , self . rwlock , self . zone_next , self . single ( ) , self . weighted ( ) , self . name , self . next , self . peer )
    }
}
...
impl ::std::fmt::Debug for ngx_http_listen_opt_t {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! ( f , "ngx_http_listen_opt_t {{ sockaddr: {:?}, addr_text: {:?}, set : {:?}, default_server : {:?}, bind : {:?}, wildcard : {:?}, ssl : {:?}, http2 : {:?}, ipv6only : {:?}, deferred_accept : {:?}, reuseport : {:?}, so_keepalive : {:?}, proxy_protocol : {:?}, backlog: {:?}, rcvbuf: {:?}, sndbuf: {:?}, fastopen: {:?}, tcp_keepidle: {:?}, tcp_keepintvl: {:?}, tcp_keepcnt: {:?} }}" , self . sockaddr , self . addr_text , self . set ( ) , self . default_server ( ) , self . bind ( ) , self . wildcard ( ) , self . ssl ( ) , self . http2 ( ) , self . ipv6only ( ) , self . deferred_accept ( ) , self . reuseport ( ) , self . so_keepalive ( ) , self . proxy_protocol ( ) , self . backlog , self . rcvbuf , self . sndbuf , self . fastopen , self . tcp_keepidle , self . tcp_keepintvl , self . tcp_keepcnt )
    }
}
...
impl ::std::fmt::Debug for ngx_http_file_cache_sh_t {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! ( f , "ngx_http_file_cache_sh_t {{ rbtree: {:?}, sentinel: {:?}, queue: {:?}, cold: {:?}, loading: {:?} }}" , self . rbtree , self . sentinel , self . queue , self . cold , self . loading )
    }
}
#[repr(C)]
